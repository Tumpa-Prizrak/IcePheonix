...
class NewCog(commands.Cog):
    ...
    async def add_voice(self, interaction: nextcord.Interaction, channel_id: int = nextcord.SlashOption(name="id")):#, description="Enter the channel name wich will be used to create a new voice сhannel")):
    ...
    @nextcord.slash_command(name="remove")
    async def remove_voice(self, interaction: nextcord.Interaction, channel_id: int = nextcord.SlashOption(name="id")):#, description="Enter the channel name wich will be used to create a new voice сhannel")):
        g = self.bot.get_channel(channel_id)
        if g == None or type(g) != VoiceChannel:
            return await interaction.send("Incorrect channel id")
