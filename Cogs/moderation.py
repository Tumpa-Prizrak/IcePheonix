...
class ModerationCommand(commands.Cog):
    ...
    async def warn(self, ctx: commands.Context, man: discord.Member, *, reason: str):
        ...
        await ctx.send(f"{man.mention} было выдано предупреждение по причине \"{reason}\" модератором {ctx.author.mention}", delete_after=h.json_data['delete_after']['command'])
    ...
    async def clear(self, ctx: commands.Context, count: int):
        await ctx.message.delete()
        if count <= 0: return await ctx.send("Параметр `count` должен быть больше нуля", delete_after=h.json_data['delete_after']['error'])
